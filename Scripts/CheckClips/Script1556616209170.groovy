...
List ids = new CropResponse().getIdsList(GlobalVariable.Clips)
...
for (i = 0; i < ids.size(); i++) {
    ...
    try {
        ...
        if (WS.verifyResponseStatusCode(PlayOut, 200)) {
			...
			ClipsPassedPatch.getClipsPassedInstance().addClipsPassedResult(passedResult)
			}else {
			failedResult.setStatus('Failed')
			ClipsFailedPatch.getClipsFailedInstance().addClipsFailedResult(failedResult)
            }
