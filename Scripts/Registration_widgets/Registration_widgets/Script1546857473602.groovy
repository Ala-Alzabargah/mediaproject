...
import shahid.api.SubmitFail as SubmitFail
	...
	code = res8.getStatusCode()
	if (code == 200) {
    def responsebody = "Registration-Widget Opened Successfully"

	String statuscode = res8.getStatusCode().toString()

	String apiname = 'Registration-widget'

	String URL = "curl -X GET \
  'https://shahid.mbc.net/widgets_git_v2/#/registration' \
  -H 'cache-control: no-cache'"
	...
	WS.verifyResponseStatusCode(res8, 200) } 
		...
		def responsebody = res8.getResponseBodyContent().toString()
			...
			String apiname = 'Registration-widgets'
