...
import shahid.api.SubmitFail as SubmitFail
    ...
    res2 = WS.sendRequest(findTestObject('null'))
	code = res2.getStatusCode()
	if (code == 200) {
    def responsebody = " Shahid.net Home Page Opened Successfully"

	String statuscode = res2.getStatusCode().toString()

	String apiname = 'Home-Page'
  ...
  -H 'cache-control: no-cache'"
	...
	WS.verifyResponseStatusCode(res2, 200) }
	
	else 
{
	def responsebody = res2.getResponseBodyContent().toString()
