...
for (i = 0; i < ids.size(); i++) {
    ...
    try {
        ...
        if (WS.verifyResponseStatusCode(PlayOut, 200)) {
			...
			EpisodesPassedPatch.getEpisodesPassedInstance().addEpisodesPassedResult(passedResult)
			}else {
			failedResult.setStatus('Failed')
			EpisodesFailedPatch.getEpisodesFailedInstance().addEpisodesFailedResult(failedResult)
            }
